...
SRC_CLIENT_WITHPROTOCOL = client_withprotocol/client.cpp
...
OBJS = $(SRCS:.cpp=.o)
TARGETS = $(BIN_DIR)/server $(BIN_DIR)/client $(BIN_DIR)/client_withprotocol 
...
$(BIN_DIR)/client_withprotocol: $(SRC_CLIENT_WITHPROTOCOL) $(SRCS_PKG) $(HEDS_PKG)
	@mkdir -p $(BIN_DIR)
	$(CC) $(CFLAGS) $(INCLUDES)  $(SRC_CLIENT_WITHPROTOCOL) $(SRCS_PKG) -o $@
