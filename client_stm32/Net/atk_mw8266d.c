...
uint8_t atk_mw8266d_exit_tcp_server()
{
    uint8_t ret;
    char cmd[64];
    
    ret = atk_mw8266d_send_at_cmd("AT+CIPCLOSE", "CLOSED", 5000);
    if (ret == ATK_MW8266D_EOK)
    {
        return ATK_MW8266D_EOK;
    }
...
/**
 * @brief       ATK-MW8266D建立TCP服务器
 *              server_port: TCP服务器端口号
 * @retval      ATK_MW8266D_EOK  : 建立TCP服务器成功
 *              ATK_MW8266D_ERROR: 建立TCP服务器失败
 */
uint8_t atk_mw8266d_set_tcp_server(char *server_port)
{
    uint8_t ret;
    char cmd[64];
								//AT+CIPSERVER=1,8086
...
uint8_t atk_mw8266d_set_AP(char *ssid, char *pwd,char *chl,char *mode)
{
	
    uint8_t ret;
    char cmd[64];
	
//								//AT+CIPSTART=”TCP”,”192.168.4.123”,8086
//    sprintf(cmd, "AT+CIPSTART=\"TCP\",\"%s\",%s", server_ip, server_port);
	
								//AT+CWSAP=”ATK-MW8266D”,”12345678”,1,4
    sprintf(cmd, "AT+CWSAP=\"%s\",\"%s\",%s,%s",  ssid,pwd,chl,mode);
    ret = atk_mw8266d_send_at_cmd(cmd, "OK", 10000);
    if (ret == ATK_MW8266D_EOK)
    {
        return ATK_MW8266D_EOK;
